...
num_codes=$(blazectl --server "$base" download Observation -q '_elements=subject&_count=1000' 2>/dev/null | jq -rc '.code' | grep -cv null | xargs)
...
num_codes=$(blazectl --server "$base" download Observation -p -q '_elements=subject&_count=1000' 2>/dev/null | jq -rc '.code' | grep -cv null | xargs)
