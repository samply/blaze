...
cat <<END
{
  ...
  "parameter": [
    {
      "name": "expression",
      "valueCode": "[Observation: Code {system: 'http://loinc.org', code: '72514-3'}] O return {patientId: First([Patient] P where P.id = Last(Split(O.subject.reference, '/')) return P.identifier.where(system = 'http://hl7.org/fhir/sid/us-ssn').value.first()), issued: O.issued, score: O.value.value}"
    }
  ]
}
END
