...
| max-concurrency | 0..1        | positiveInt | The maximum number of concurrent reader threads of the rand-read sweep, between 1 and 1024. Defaults to 32.   |
...
The \$disk-perf operation can be executed using [blazectl][1] v1.5 or later, which provides the `disk-perf` subcommand. It submits the request, polls the async status endpoint and prints the results.
