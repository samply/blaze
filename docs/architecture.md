...
Blaze doesn't just guarantee the *isolation level* Serializable, it achieves it through **actual serial execution**: all transactions are literally run one at a time. Many databases provide the Serializable isolation level while still executing transactions concurrently (e.g. via two-phase locking or serializable snapshot isolation) and only ensure the *outcome* is equivalent to some serial order. Blaze instead maintains a single Transaction Log with a total order, and the Indexer applies transactions one after another in exactly that order. Transactions are never executed concurrently and their effects never interleave.

Every write interaction becomes a transaction that is placed into this single, totally ordered log. A [create](api/interaction/create.md), [update](api/interaction/update.md), [delete](api/interaction/delete.md), [conditional delete](api/interaction/delete-type.md) or [delete history](api/interaction/delete-history.md) each form one transaction. A whole [transaction](api/interaction/transaction.md) bundle also forms a single transaction — all of its actions are applied together atomically. A [batch](api/interaction/batch.md) bundle is the exception: it is not a transaction itself, each of its actions becomes its own separate transaction in the log.

The practical consequence of this design is that **a transaction can only start once the previous transaction has finished**. A large transaction (for example a transaction bundle creating or updating many thousands of resources) therefore **blocks all smaller transactions** submitted after it until it completes. Those smaller transactions are not lost or rejected — they simply wait their turn behind the large one. This is a deliberate trade-off: it makes reasoning about consistency trivial and keeps the implementation simple, at the cost of write throughput being bounded by strictly serial execution. To avoid head-of-line blocking of latency-sensitive writes, keep individual transactions small and, where possible, submit very large data loads during periods of low interactive load.
