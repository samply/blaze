...
#### `DB_RESOURCE_CACHE_SIZE` <Badge type="warning" text="Since 0.8"/> <Badge type="danger" text="Deprecated 1.5.0"/>

The size of the resource cache of the DB in number of resources. (Deprecated) Replaced with `DB_RESOURCE_CACHE_SIZE_RATIO`.

**Default:** 100000

#### `DB_RESOURCE_CACHE_SIZE_RATIO` <Badge type="warning" text="Since 1.5.0"/>
...
#### `EXTERN_TERMINOLOGY_SERVICE_URL` <Badge type="warning" text="Since 1.3.0"/>

Terminology service URL to make terminology operations available in [CQL Queries](../cql-queries.md).

Using an external terminology service is exclusive to using the internal terminology service via `ENABLE_TERMINOLOGY_SERVICE`. Please use either the internal or the external terminology service.

#### `EXTERN_TERMINOLOGY_SERVICE_CLIENT_TRUST_STORE` <Badge type="warning" text="Since 1.3.0"/>
...
#### `EXTERN_TERMINOLOGY_SERVICE_CLIENT_TRUST_STORE_PASS` <Badge type="warning" text="Since 1.3.0"/>

The password for the PKCS #12 trust store.

#### `EXTERN_TERMINOLOGY_SERVICE_OPENID_PROVIDER_URL` <Badge type="warning" text="Since 1.7.0"/>

[OpenID Connect][4] provider URL to enable authentication against the external terminology service using the OAuth2 Client Credentials flow. Setting this variable activates the feature.

#### `EXTERN_TERMINOLOGY_SERVICE_OPENID_CLIENT_ID` <Badge type="warning" text="Since 1.7.0"/>

OAuth2 client ID used for the Client Credentials flow against the external terminology service.

#### `EXTERN_TERMINOLOGY_SERVICE_OPENID_CLIENT_SECRET` <Badge type="warning" text="Since 1.7.0"/>

OAuth2 client secret used for the Client Credentials flow against the external terminology service.

#### `EXTERN_TERMINOLOGY_SERVICE_OPENID_CLIENT_TRUST_STORE` <Badge type="warning" text="Since 1.7.0"/>

A PKCS #12 trust store containing CA certificates needed for the [OpenID Connect][4] provider of the external terminology service. The PKCS #12 trust store has to be generated by the Java keytool. OpenSSL will not work.
...
#### `EXTERN_TERMINOLOGY_SERVICE_OPENID_CLIENT_TRUST_STORE_PASS` <Badge type="warning" text="Since 1.7.0"/>

The password for the PKCS #12 trust store.

#### `EXTERN_VALIDATOR_URL` <Badge type="warning" text="Since 1.11.0"/>

Base URL of an external FHIR validator like the [MII FHIR Validator][8]. Setting this variable enables external profile validation: every incoming resource of a create, update or transaction/batch interaction is forwarded to `<EXTERN_VALIDATOR_URL>/validateResource` for schema and profile validation before persistence.

#### `VALIDATOR_FAILURE_MODE` <Badge type="warning" text="Since 1.11.0"/>
...
If the external validator is unreachable, the write is always rejected with `503 Service Unavailable` (fail closed).

**Default:** `tag-outcome`

#### `EXTERN_VALIDATOR_MAX_CONCURRENCY` <Badge type="warning" text="Since 1.11.0"/>

The maximum number of validation requests sent to the external validator concurrently. Requests beyond this limit are queued until a slot becomes available. Lower this value to protect a validator that can't handle many parallel requests; raise it to increase throughput.

**Default:** `4`

#### `EXTERN_VALIDATOR_CLIENT_TRUST_STORE` <Badge type="warning" text="Since 1.11.0"/>

A PKCS #12 trust store containing CA certificates needed for the external validator. The PKCS #12 trust store has to be generated by the Java keytool. OpenSSL will not work.
...
#### `EXTERN_VALIDATOR_CLIENT_TRUST_STORE_PASS` <Badge type="warning" text="Since 1.11.0"/>
