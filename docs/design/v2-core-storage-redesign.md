...
- The index is **authoritative** — the index DB is operated durably regardless of the resource store's placement, via the per-transaction Layer-3 sync (see *Per-resource WriteBatch*).
...
**Why `internal-id` orders the CF, with no type prefix.** Because `internal-id` carries `t` in its high bits (see [Internal ID Format](#internal-id-format)), ordering the CF by `internal-id` orders it by transaction time. Two properties follow that a type-major ordering (a `tid` prefix, clustering all resources of a type together) would forfeit:

- **Creates append at the tail of the keyspace** — the low-write-amplification regime for leveled compaction — even when a single mixed-type bundle writes many resource types at once. A `tid` prefix would turn each such transaction into a full-keyspace scatter (every type's range touched at once), so every memtable flush spans the whole keyspace and overlaps all of L1, multiplying compaction cost on the dominant bulk-import path.
- **Time-ordered body scans stay sequential** — state sync, system bulk export by `t`, and system `_history` all stream bodies in `t` order. A `tid` prefix would scatter these across per-type ranges.

The per-type block compression a `tid` prefix would add is largely already captured by the trained zstd dictionary, which models cross-type FHIR structure globally, and the worst block-pollution source — large `base64Binary` payloads — is already removed to the `Base64BinaryValue` CF. The layout also stays congruent with `ResourceAsOf` (likewise `internal-id`-keyed), so a resolved handle point-reads its body by the same id.

The value is FHIR CBOR body bytes with four storage-layer transforms applied at write time and inverted at read time. The transforms are independent of the rewrite stage (which produces the canonical post-rewrite body); they operate on the bytes that body would otherwise serialize to, and never affect command shape, transport encoding, content equality for version dedup, or any index key.
...
Layer 2 (tx-index batch — `ResourceAsOf`, `TypeAsOf`, `SystemAsOf`, `LogicalIdToInternalId`, `InternalIdToLogicalId`, `TypeStats`, `SystemStats`) and Layer 3 (`TxSuccess` + `TByInstant` visibility gate) are unchanged in shape; only their position relative to body writes is now formalised. The sync discipline is part of the layering: Layer 1 and Layer 2 batches are written unsynced, and Layer 3 is the transaction's only synced write (`sync = true`). With one index DB there is one sequential WAL, so the Layer-3 sync durably flushes every earlier batch of the transaction in a single fsync — the visibility gate doubles as the durability gate. This is load-bearing in v2: the index is authoritative and tx-log retention is bounded, so anything visible through `head(TxSuccess)` must already be durable; unsynced Layer-1/Layer-2 writes are safe to lose precisely because the `TxSuccess` row that would make them visible is lost with them. Crash semantics carry over: Layer 1 commits that survive past a crash without Layer 3 sit as inert rows invisible to readers (which gate on `head(TxSuccess)`). Bodies now contribute to that inert footprint, but distributed re-apply overwrites by key (same `(internal-id, num-changes)`), and standalone's inert leak per crashed-but-uncommitted transaction is bounded by Kafka-free single-writer semantics.
...
### Large primitive values (`Base64BinaryValue` CF)

`base64Binary` primitive values — `Attachment.data`, `Binary.data`, `Signature.data` — are the one place a FHIR resource carries an arbitrarily large, high-entropy payload (an embedded PDF, image, or CDA document). Left inline in the `Resource` CF they defeat that CF's design on every axis: a multi-MB value is rewritten on every compaction it passes through (write amplification on the largest dataset), the trained dictionary buys nothing on high-entropy bytes, and a `_summary` / `_elements` read that never asked for the payload still pays to read and decompress it.

v2 extracts large `base64Binary` values into a dedicated **`Base64BinaryValue` column family** and leaves a placeholder in the body, reusing the **CBOR major-type dispatch** of *References as internal ids*: the schema-aware encoder knows a `base64Binary` slot's value is normally a CBOR text string (major type 3), and writes a fixed-length byte string (major type 2) — the value's content hash — in its place. The decoder dispatches on the major type at that slot: text string → the value is inline (small, below threshold), content-hash byte string → fetch from `Base64BinaryValue`.

**Stored as raw bytes, not base64.** FHIR transports `base64Binary` as base64 text — a flat +33% over the raw bytes, and the common payloads (already-compressed PDF/JPEG) recover none of it under compression. The extracted value is stored decoded, as a CBOR byte string. (Blaze's internal model currently holds the value as a base64 `String`; the byte-string form is the stored representation.)

**Key — `shard-id (2), internal-id (8), content-hash (16)`.** The hash is taken over the raw bytes (a 128-bit non-cryptographic hash such as xxh3-128; collision probability is negligible for these populations). The keying yields three properties:

- **Threshold spill.** Only values whose raw (decoded) size is at least **4 KB** are extracted; smaller `base64Binary` values — signatures, short inline notes — stay inline in the body, where they still compress, share the trained dictionary, and incur no extra point read. The threshold is a **fixed constant, not a configuration option**. 4 KB sits in the trough between the small-value cluster (sub-2 KB signatures and notes) and real document payloads (tens of KB to many MB), is comfortably above BlobDB's efficient floor, and is a quarter of the `Resource` block size so a sub-threshold inline value never dominates a block. The size is measured on the decoded bytes (base64 text is ≈ 1.33× larger).
- **Cross-version deduplication, for free.** The key omits `num-changes`, so every version of a resource shares one keyspace: an attachment carried unchanged across versions — a `put` that edits metadata but not the embedded document, the common case — hashes to the same key and is stored once. The write is idempotent; no reference counting is needed on the write path. The body's hash placeholder also keeps version dedup (rewrite step 4) correct through the extraction — an attachment change yields a different hash, hence a different body, hence a new version; an unchanged attachment yields an identical placeholder.
- **Local, on-demand erasure.** Because the key is prefixed by `internal-id`, `patient-purge` removes a patient's payloads with the same `shard-id`-prefixed `RangeDelete` that clears its other sharded rows. `delete-history` deletes the payloads referenced only by the removed versions: the surviving set is derived on demand by reading the (now small) surviving version bodies under that `internal-id` and collecting their placeholders. There is no persistent ref-count and no cross-resource or cross-shard bookkeeping.

**Materialization.** A read translates each placeholder back with one point read into `Base64BinaryValue` and re-inserts the value at the slot. Projections that exclude the payload (`_summary`, `_elements` without the data element, a `DocumentReference` search that doesn't return content) skip the fetch entirely — lazy loading an inline payload cannot offer. The materialized-resource cache holds the assembled form, so a hot read pays the fetch at most once.

**CF options.** `Base64BinaryValue` runs BlobDB (`enable-blob-files?` with `min-blob-size` at or below the 4 KB extraction threshold, so every extracted value lands in a blob file and the CF's LSM holds only pointers) so values live in blob files and are not rewritten on every LSM compaction — the direct fix for the write-amplification problem above. It carries no trained dictionary (useless on high-entropy bytes) and no block cache (payloads are large, cold, and cached above as part of the materialized resource), with a larger block size than the metadata CFs. The `shard-id` prefix places each payload on its resource version's owner shard, so shard moves and per-shard state-sync range over it exactly as for the `Resource` CF.

### Trained zstd dictionary on the Resource CF

Small records compress poorly on their own: a compressor "learns" from the bytes that precede each match, and a single ~1 KB body has almost no "past" to build on. A **compression dictionary** supplies that missing context — a block of representative bytes the compressor can reference from the first byte of every record. For a corpus of structurally similar records — every FHIR body repeats the same field-name sequences, `system` URLs, and CBOR map prologues — the win is large; zstd's own small-record benchmark reports dictionaries improving the ratio several-fold while also speeding both compression and decompression ([facebook/zstd — "The case for Small Data compression"](https://github.com/facebook/zstd#the-case-for-small-data-compression)). On a small-record CF, 20–40% on top of block-only zstd is typical.

**How the dictionary is built — per SST, by RocksDB, automatically.** RocksDB does *not* accept an externally supplied fixed dictionary; its public `CompressionOptions` has no field for one. Instead, when `zstd_max_train_bytes > 0` and `max_dict_bytes > 0`, RocksDB samples up to `zstd_max_train_bytes` of data blocks from each SST as it is written (at flush and compaction), runs zstd's dictionary trainer over those samples to produce a dictionary of at most `max_dict_bytes`, and stores it in that SST's meta-block. Every block in the SST is compressed against that dictionary, and at read time the dictionary is loaded from the meta-block — pinned in memory, or cached alongside index/filter blocks — so decompression is self-contained ([RocksDB — Dictionary Compression](https://github.com/facebook/rocksdb/wiki/Dictionary-Compression)). `use_zstd_dict_trainer` selects between zstd's `ZDICT` trainer (the default, best ratio) and a cheaper non-trainer construction.

The trainer is zstd's `ZDICT_trainFromBuffer` (the COVER / fastCover family): it scans the samples for the substrings that recur most across records and packs the highest-value ones into the dictionary. zstd's guidance is a dictionary of ~100 KB trained on samples totalling ~100× that size, drawn from at least a few thousand records ([zstd `zdict.h`](https://github.com/facebook/zstd/blob/dev/lib/zdict.h)). For this CF a smaller dictionary (`max_dict_bytes` ≈ 16 KB) over a per-SST sample budget on the order of 100 KB–1 MB (`zstd_max_train_bytes`) is the starting point, to be tuned against the benchmarks.

**Why per-SST training fits here, rather than a shipped artifact.** The dictionary always reflects the *actual* stored data — a deployment's real resource mix and IGs — not a corpus frozen at build time; each SST stays self-describing (no external dictionary file to ship, version, or keep in sync with the data); and it evolves automatically as compaction rewrites SSTs. zstd stamps a dictionary id into every frame, so SSTs trained with different dictionaries coexist on disk with no migration ([RFC 8878 — Zstandard Compression](https://www.rfc-editor.org/rfc/rfc8878)).

These options are not yet exposed by Blaze's RocksDB wrapper and are tracked as wrapper work — see *Column-family options*.

### Column-family options

The `Resource` CF departs from the metadata-CF defaults along three axes — **sizing** (it is the largest CF in the Index DB), **filtering** (every read is a guaranteed hit), and **compression** (trained zstd). The recommended options, in the EDN shape Blaze uses to configure a column family:

```clojure
{;; Largest CF in the Index DB, holding every resource body. Rows are
 ;; write-once-immutable — a new version is a new (internal-id, num-changes)
 ;; key; an existing key is never overwritten until delete-history /
 ;; patient-purge. That append-mostly, no-key-churn profile favours fewer,
 ;; larger files to minimise write amplification over the biggest dataset.
 :write-buffer-size-in-mb 128
 :max-bytes-for-level-base-in-mb 512        ; keep the ×4 ratio to write-buffer
 :target-file-size-base-in-mb 128

 ;; A ~16 KB block holds a dozen-plus post-elision bodies. With no block cache
 ;; (see Block cache strategy) block size is purely a compression-ratio vs
 ;; point-read-decode knob; the trained dictionary supplies cross-block
 ;; context, so blocks need not be inflated for ratio.
 :block-size #blaze/var db-block-size

 ;; Every Resource read is keyed by an (internal-id, num-changes) that a
 ;; preceding ResourceAsOf row already proved exists — the body store is never
 ;; probed for a missing key. optimize-filters-for-hits drops the bottommost
 ;; level's bloom filters (the bulk of all keys) at zero false-negative cost;
 ;; the retained upper-level filters route a point read past L0..Ln-1 to the
 ;; bottom level where the body lives.
 :bloom-filter? true
 :optimize-filters-for-hits? true

 ;; Recently written bodies are read back almost immediately — transaction
 ;; response rendering, and the rewrite stage's version-dedup byte-compare
 ;; (Rewrite Stage, step 4) which point-reads the current version's body, often
 ;; still in the memtable.
 :memtable-whole-key-filtering? true

 ;; Trained zstd dictionary (see Trained zstd dictionary on the Resource CF).
 ;; Must be set on BOTH the regular and the bottommost compression options:
 ;; RocksDB does not apply compression_opts to the bottommost level — where the
 ;; overwhelming majority of bodies live under the ZSTD-bottommost default —
 ;; unless bottommost_compression_opts is explicitly enabled.
 :zstd-dictionary-max-bytes 16384           ; 16 KiB dictionary
 :zstd-max-train-bytes 102400}              ; 100 KiB of samples per SST
```

Inherited unchanged from the defaults shared by every CF: leveled compaction with `level_compaction_dynamic_level_bytes` (a well-shaped LSM with low space amplification on a large CF) and LZ4 on the upper levels with ZSTD on the bottommost — the dictionary above attaches to the ZSTD levels.

Relative to the v1 standalone resource store, the filtering flags (`bloom-filter?`, `optimize-filters-for-hits?`, `memtable-whole-key-filtering?`) carry over unchanged but rest on a stronger guarantee: v1 relied on "callers only fetch content-hashes they computed," whereas v2 reads are gated by a `ResourceAsOf` row that proves the `(internal-id, num-changes)` exists, so a miss is structurally impossible. The sizing knobs increase (v1 used 64 / 256 / 64) because the CF now absorbs the entire resource store and is the Index DB's largest dataset; these three values are starting points to be confirmed against the perf benchmarks (see *Verification*), while the filtering and compression choices are structural and follow from the access pattern.

Two of these options are not yet expressible by the RocksDB wrapper and are tracked as wrapper work, separate from the core-storage change:

- **No block cache for this CF specifically.** The `Resource` CF opens with `setNoBlockCache(true)` while the metadata CFs share a block cache within the one Index DB (see *Block cache strategy*). The wrapper currently assigns a single block cache uniformly to every CF of a DB; realising the split needs a per-CF opt-out.
- **The trained-dictionary options** (`zstd-dictionary-max-bytes` / `zstd-max-train-bytes`) have no wrapper equivalent yet; they map to RocksDB `CompressionOptions` (`max_dict_bytes` / `zstd_max_train_bytes`), set on both the regular and bottommost compression options as noted above.
...
- **Sharded** CFs: `Resource`, `Base64BinaryValue`, `SearchParamValueResource`, `ResourceSearchParamValue`, `CompartmentResource`, `CompartmentSearchParamValueResource`, `PatientLastChange`, `ResourceReference`, `ReferenceResource`.
...
| Base64BinaryValue (new CF in Index DB) | —                                                                                 | `shard-id (2), internal-id (8), content-hash (16)`                                                 | —                                                            | raw `base64Binary` bytes (CBOR byte string)        | Large `base64Binary` payloads extracted from the body; content-hash keyed for cross-version dedup; BlobDB, no dictionary, no block cache; sharded by version owner. See *Large primitive values* |
...
| Base64BinaryValue (CF in Index DB)  | —                                         | 26                    | —                 | raw value bytes   |
...
- **RocksDB value-size limits for non-`base64Binary` content.** Large `base64Binary` payloads are extracted to the `Base64BinaryValue` CF (see *Large primitive values*), so the embedded-document case no longer bloats the `Resource` CF. A single rewritten body could still in principle approach unhealthy RocksDB value sizes through non-binary content (a pathologically large narrative, or a resource with very many elements); operator-enforced per-resource size caps remain the fallback. Minor in practice.
- **Incremental maintenance of large enumerated `Group` / `List`.** FHIR R5 defines `$add` / `$remove` / `$filter` on `Group.member` and `List.entry` for maintaining very large enumerated collections without round-tripping the whole resource (`$filter` returns a `SUBSETTED` projection). **API-layer support is in scope and decouples from storage:** the operations can apply against the normal version-full-body model — the client sends only the delta, and `$filter` reuses the summary / `SUBSETTED` projection path — giving the network and ergonomics win with no storage change. What is **deferred** is *O(delta) storage*: under the full-body model an `$add` to an N-member collection rewrites an N-member body, so building a large collection incrementally is O(N²). The principled fix mirrors `ResourceAsOf` at sub-element granularity — store members as one row each with a validity interval, keyed `(shard-id, group-internal-id, member-identity)`, so `$add` writes one row, `$remove` closes one interval, and the member set at any `t` is an interval-filtered range scan, while the `Group` / `List` scalar fields keep versioning normally in the `Resource` CF. That is a real subsystem (touching version reconstruction, `_history` / `vread`, search, materialization, dedup, purge) for two resource types, so it is gated on a demonstrated workload of frequently-mutated very large enumerated collections. A criteria-based `Group` (`Group.characteristic`) or a CQL / query cohort is usually the better model for population-scale membership than an enumerated `member` list, which further narrows when the temporal-member store is worth building. `CodeSystem.concept` is **not** in scope here: concept *query* is served by the terminology subsystem's own graph/index rather than the `Resource` CF, concepts load wholesale (or live externally via `content`), FHIR defines no incremental concept operation, and `concept` is already skipped on summary reads (it is not `isSummary`).
