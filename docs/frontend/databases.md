...
Right above the column families, the database page shows the results of the most recent [disk performance measurement](../performance/disk-perf.md#built-in-measurement) of the volume the database directory resides on. Four figures summarize the run: the overall score with its rating, the best random read IOPS together with the concurrency it was reached at, the sequential write throughput and the fsyncs per second. Below them, a **Random Reads** section plots the random read IOPS per concurrency against the reference curve of a good local NVMe SSD as well as the read latency percentiles per concurrency, followed by a table listing the IOPS, throughput and latency percentiles of each concurrency. The line below the heading links to the job that produced the results.
