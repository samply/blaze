...
Download the most recent version https://github.com/life-research/blaze/releases/tag/v0.7.0-alpha.8[here].
Look for `blaze-0.7.0-alpha.8-standalone.jar`.
...
```
DATABASE_URI=datomic:free://localhost:4334/db java -server -Xms2g -Xmx2g -XX:+UseG1GC -jar blaze-0.7.0-alpha.8-standalone.jar
```
