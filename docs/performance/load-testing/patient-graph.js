...
function fetchGraph(id) {
  return http.get(`${base}/Patient/${id}/$graph?graph=patient-observation-encounter`, graphParams);
}
...
export function setup() {
  ...
  const sampledPatientIds = patientIds.filter(id => Math.random() < 0.1);

  console.log(`sampled ${sampledPatientIds.length} random patient IDs (10 %)`);

  console.log('pre-fetch all graphs to fill caches...');

  for (const id of sampledPatientIds) {
    const resp = fetchGraph(id);
