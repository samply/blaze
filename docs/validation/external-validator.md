...
The MII FHIR Validator derives its terminology configuration from `TX_SERVER`,
generating the `fhir-settings.json` the validator needs on startup. Plain HTTP
terminology servers are handled by that, so no further configuration is
required.
