...
(defn- group-by-estimated-scan-size [batch-db tid clauses]
  (let [sized-clauses (mapv #(vector % (estimated-scan-size batch-db tid %)) clauses)
...
(defn type-query-plan
  ...
  (if (sort-clause? (first clauses))
    ...
    (let [[clauses other-clauses] (split-clauses batch-db tid clauses)]
      (if (seq clauses)
        {:query-type :type
         :scan-clauses (mapv clause-stats clauses)
         :seek-clauses (mapv clause-stats other-clauses)}
        {:query-type :type
