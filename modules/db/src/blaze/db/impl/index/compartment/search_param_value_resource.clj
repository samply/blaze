...
(defn- key-size
  ...
  (^long [co-res-id value id]
   (+ (key-size co-res-id value) 1 (bs/size id))))
  ...
  ([compartment sp-c-hash tid value]
   (let [co-c-hash (coll/nth compartment 0)
     ...
     (-> (bb/allocate (key-size co-res-id value))
         ...
         bb/flip!
         bs/from-byte-buffer!)))
  ([compartment sp-c-hash tid value id]
   (let [co-c-hash (coll/nth compartment 0)
         co-res-id (coll/nth compartment 1)]
     (-> (bb/allocate (key-size co-res-id value id))
         (bb/put-int! co-c-hash)
         (bb/put-null-terminated-byte-string! co-res-id)
         (bb/put-int! sp-c-hash)
         (bb/put-int! tid)
...
(defn- index-handles* [snapshot seek-key]
  (coll/eduction
   u/by-id-grouper
   (i/prefix-keys
    snapshot
...
(defn index-handles
  ...
  ([snapshot compartment c-hash tid value]
   (index-handles* snapshot (encode-seek-key compartment c-hash tid value)))
  ([snapshot compartment c-hash tid value start-id]
   (index-handles* snapshot (encode-seek-key compartment c-hash tid value start-id))))
