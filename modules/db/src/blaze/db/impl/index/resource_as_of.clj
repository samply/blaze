...
(defn- type-entry-creator
  ...
  `kb` at the time of invocation is less than or equal `base-t` and greater
  than or equal `since-t`."
...
(defn instance-history
  ...
  starting at `start-t`."
  {:arglists '([batch-db tid id start-t])}
  [{:keys [snapshot t since-t]} tid id start-t]
...
(defn resource-handle
  ...
  found."
  {:arglists '([batch-db tid id])}
  [{:keys [snapshot t since-t]} tid id]
