...
(defn estimated-storage-size
  "Returns an estimation of the storage size in bytes for all keys with the
  prefix consisting of `c-hash`, `tid` and `value`.

  Returns an anomaly if estimating the storage size isn't supported by
  `kv-store`.
  ...
  (let [seek-key (encode-seek-key c-hash tid value)
    ...
    (kv/estimate-storage-size kv-store :search-param-value-index key-range)))
