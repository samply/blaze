...
(defn- encode-seek-key-for-prev
  ...
  ([c-hash tid value did]
   (bs/concat (encode-seek-key c-hash tid value did) max-hash-prefix)))
...
(defn all-keys-prev!
  ...
  ([iter c-hash tid]
   (let [prefix (encode-seek-key c-hash tid)
     ...
     (i/prefix-keys-prev! iter prefix decode-value-did-hash-prefix start-key)))
  ([iter c-hash tid start-value start-did]
   (let [prefix (encode-seek-key c-hash tid)
         start-key (encode-seek-key-for-prev c-hash tid start-value start-did)]
     (i/prefix-keys-prev! iter prefix decode-value-did-hash-prefix start-key))))
