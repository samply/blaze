...
(defn estimated-storage-size
  ...
  `search-param` with `compiled-values` under `tid`.
  ...
  (transduce
   (comp (map #(p/-estimated-storage-size search-param batch-db tid modifier %))
...
(defn index-handles
  ...
  The index handles are not distinct for more than one compiled value and not
  ordered by id in any case."
  ([search-param batch-db tid modifier compiled-values]
...
(defn ordered-index-handles
  ...
  ([search-param batch-db tid modifier compiled-values]
   (if (= 1 (count compiled-values))
     (p/-ordered-index-handles search-param batch-db tid modifier
                               (first compiled-values))
     (->> (map #(p/-ordered-index-handles search-param batch-db tid modifier %)
               compiled-values)
          (apply coll/union ih/union))))
  ([search-param batch-db tid modifier compiled-values start-id]
   (if (= 1 (count compiled-values))
     (p/-ordered-index-handles search-param batch-db tid modifier
                               (first compiled-values) start-id)
     (->> (map #(p/-ordered-index-handles search-param batch-db tid modifier %
                                          start-id)
               compiled-values)
          (apply coll/union ih/union)))))
