  ...
  (with-system-data [{:blaze.db/keys [node]} config]
    ...
    (testing "with ValueSet expansion errors"
      (with-redefs [ts/expand-value-set (fn [_ _] (ac/completed-future (ba/fault "msg-125400")))]
        (testing "type query"
          (given (pull-type-query node "Condition" [["code:in" "http://fhir.org/VCL?v1=(http://fhir.de/CodeSystem/bfarm/icd-10-gm)concept<<C69"]])
            ::anom/category := ::anom/fault
            ::anom/message := "Error while expanding the ValueSet `http://fhir.org/VCL?v1=(http://fhir.de/CodeSystem/bfarm/icd-10-gm)concept<<C69`. Cause: msg-125400"))))))
