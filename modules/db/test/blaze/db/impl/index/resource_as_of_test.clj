  ...
  (testing "one purged patient"
    (with-system-data [{:blaze.db/keys [node]} config]
      ...
      (testing "returns one delete entry at t=2"
        (with-open [snapshot (kv/new-snapshot (:kv-store node))]
          (given (rao/prune snapshot 10 2)
            [:delete-entries count] := 1
            [:delete-entries 0 0] := :resource-as-of-index
            [:delete-entries 0 1 rao-tu/decode-key] := {:type "Patient" :id "0" :t 1}
            :num-entries-processed := 1
            :next := nil)))))
    ...
    (with-system-data [{:blaze.db/keys [node]} config]
      ...
      (testing "returns one delete entry at t=3 and n=1"
        ...
        (testing "it's possible to continue with the next entry"
          (with-open [snapshot (kv/new-snapshot (:kv-store node))]
            (given (rao/prune snapshot 2 3 (codec/tid "Patient") (codec/id-byte-string "1") 1)
              [:delete-entries count] := 1
              [:delete-entries 0 0] := :resource-as-of-index
              [:delete-entries 0 1 rao-tu/decode-key] := {:type "Patient" :id "1" :t 1}
              :num-entries-processed := 1
              :next := nil))))

      (testing "returns two delete entries at t=3"
        (with-open [snapshot (kv/new-snapshot (:kv-store node))]
          (given (rao/prune snapshot 10 3)
            [:delete-entries count] := 2
            [:delete-entries 0 0] := :resource-as-of-index
            [:delete-entries 0 1 rao-tu/decode-key] := {:type "Patient" :id "0" :t 1}
            [:delete-entries 1 0] := :resource-as-of-index
            [:delete-entries 1 1 rao-tu/decode-key] := {:type "Patient" :id "1" :t 1}
            :num-entries-processed := 2
            :next := nil))))))
