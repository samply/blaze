...
(deftest init-test
  ...
  (testing "invalid max-size"
    (given-failed-system (assoc-in config [:blaze.db/resource-cache :max-size-in-bytes] ::invalid)
      :key := :blaze.db/resource-cache
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :via] := [::rc/max-size-in-bytes]
      [:cause-data ::s/problems 0 :val] := ::invalid)))
...
(deftest stats-test
  ...
  (testing "with zero max size"
    (with-system [{cache :blaze.db/resource-cache store ::rs/kv}
                  (assoc-in config [:blaze.db/resource-cache :max-size-in-bytes] 0)]
