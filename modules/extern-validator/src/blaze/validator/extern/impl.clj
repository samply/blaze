...
(def validation-status-system
  ...
  "https://samply.github.io/blaze/fhir/CodeSystem/ValidationStatus")

(def outcome-extension-url
  "URL of the meta extension referencing the contained OperationOutcome."
  "https://samply.github.io/blaze/fhir/StructureDefinition/validation-outcome")

(def contained-outcome-id
  "Id of the contained OperationOutcome of invalid resources."
  "validation-outcome")
...
(def ^:private outcome-extension
  (type/extension
   {:url outcome-extension-url
    :value (type/reference {:reference (type/string (str "#" contained-outcome-id))})}))
...
(defn tag-invalid
  ...
  When `contain-outcome?` is true, additionally embeds `operation-outcome` as a
  contained resource referenced from a meta extension."
  [resource operation-outcome contain-outcome?]
  (let [meta (update (meta-map (:meta resource)) :tag (fnil conj []) invalid-coding)]
    (if contain-outcome?
      (-> resource
          (assoc :meta (type/meta (update meta :extension (fnil conj []) outcome-extension)))
          (update :contained (fnil conj []) (assoc operation-outcome :id contained-outcome-id)))
