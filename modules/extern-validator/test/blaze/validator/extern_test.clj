...
(deftest init-test
  ...
  (testing "missing parsing-context"
    (given-failed-system (update config :blaze.validator/extern dissoc :parsing-context)
      :key := :blaze.validator/extern
      :reason := ::ig/build-failed-spec
  ...
  (testing "invalid max-concurrency"
    (given-failed-system (assoc-in config [:blaze.validator/extern :max-concurrency] 0)
      :key := :blaze.validator/extern
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :via] := [::extern/max-concurrency]
...
(deftest validate-test
  ...
  (testing "an invalid resource is tagged with contained outcome (tag-outcome)"
    (with-system [{validator :blaze.validator/extern ::keys [http-client]} config]
      ...
      (let [resource @(validator/validate validator patient)]
        (given resource
          :fhir/type := :fhir/Patient
          [:meta :tag 0 :code] := #fhir/code "invalid"
          [:contained 0 :fhir/type] := :fhir/OperationOutcome)
        (testing "the meta extension references the contained outcome by its id"
          (is (= (type/string (str "#" (get-in resource [:contained 0 :id])))
                 (get-in resource [:meta :extension 0 :value :reference])))))))
  ...
  (testing "a concurrency-bounded validator validates all resources"
    (with-system [{validator :blaze.validator/extern ::keys [http-client]}
                  (assoc-in config [:blaze.validator/extern :max-concurrency] 1)]
      (stub-validator http-client valid-outcome)

      (doseq [i (range 5)]
        (given @(validator/validate validator (assoc patient :id (str i)))
          :fhir/type := :fhir/Patient
          :id := (str i))))))
