...
public final class DataRequirement extends AbstractElement implements Complex, ExtensionValue {
     ...
     * 4 byte - subject reference
     * 4 byte - mustSupport reference
     * 4 byte - codeFilter reference
     * 4 byte - dateFilter reference
     * 4 byte - limit reference
     * 4 byte - sort reference
     * 4 byte - padding
     */
    ...
    private static final Keyword SUBJECT_CODEABLE_CONCEPT = RT.keyword(null, "subjectCodeableConcept");
    private static final Keyword SUBJECT_REFERENCE = RT.keyword(null, "subjectReference");
    ...
    private static final Keyword[] FIELDS = {ID, EXTENSION, TYPE, PROFILE, SUBJECT_CODEABLE_CONCEPT, SUBJECT_REFERENCE, MUST_SUPPORT, CODE_FILTER, DATE_FILTER, LIMIT, SORT};
    ...
    private final ExtensionValue subject;
    ...
    private DataRequirement(ExtensionData extensionData, Code type, List<Canonical> profile, ExtensionValue subject, List<String> mustSupport, List<CodeFilter> codeFilter, List<DateFilter> dateFilter, PositiveInt limit, List<Sort> sort) {
        super(extensionData);
        this.type = type;
    ...
    public static DataRequirement create(IPersistentMap m) {
        Object subject = m.valAt(SUBJECT_CODEABLE_CONCEPT);
        if (subject == null) {
            subject = m.valAt(SUBJECT_REFERENCE);
        }
        return new DataRequirement(ExtensionData.fromMap(m), (Code) m.valAt(TYPE),
                Base.listFrom(m, PROFILE),
                (ExtensionValue) subject,
                Base.listFrom(m, MUST_SUPPORT),
                Base.listFrom(m, CODE_FILTER),
                Base.listFrom(m, DATE_FILTER),
                (PositiveInt) m.valAt(LIMIT),
                Base.listFrom(m, SORT));
    }
    ...
    public Object valAt(Object key, Object notFound) {
        ...
        if (key == SUBJECT_CODEABLE_CONCEPT) return subject instanceof CodeableConcept ? subject : null;
        if (key == SUBJECT_REFERENCE) return subject instanceof Reference ? subject : null;
    ...
    public ISeq seq() {
        ...
        if (subject instanceof CodeableConcept) {
            seq = appendElement(seq, SUBJECT_CODEABLE_CONCEPT, subject);
        } else if (subject instanceof Reference) {
            seq = appendElement(seq, SUBJECT_REFERENCE, subject);
        }
    ...
    public DataRequirement assoc(Object key, Object val) {
        if (key == TYPE) return new DataRequirement(extensionData, (Code) val, profile, subject, mustSupport, codeFilter, dateFilter, limit, sort);
        if (key == PROFILE) return new DataRequirement(extensionData, type, (List<Canonical>) val, subject, mustSupport, codeFilter, dateFilter, limit, sort);
        if (key == SUBJECT_CODEABLE_CONCEPT || key == SUBJECT_REFERENCE) return new DataRequirement(extensionData, type, profile, (ExtensionValue) val, mustSupport, codeFilter, dateFilter, limit, sort);
        if (key == MUST_SUPPORT) return new DataRequirement(extensionData, type, profile, subject, (List<String>) val, codeFilter, dateFilter, limit, sort);
        if (key == CODE_FILTER) return new DataRequirement(extensionData, type, profile, subject, mustSupport, (List<CodeFilter>) val, dateFilter, limit, sort);
        if (key == DATE_FILTER) return new DataRequirement(extensionData, type, profile, subject, mustSupport, codeFilter, (List<DateFilter>) val, limit, sort);
        if (key == LIMIT) return new DataRequirement(extensionData, type, profile, subject, mustSupport, codeFilter, dateFilter, (PositiveInt) val, sort);
        if (key == SORT) return new DataRequirement(extensionData, type, profile, subject, mustSupport, codeFilter, dateFilter, limit, (List<Sort>) val);
        if (key == EXTENSION) return new DataRequirement(extensionData.withExtension(val), type, profile, subject, mustSupport, codeFilter, dateFilter, limit, sort);
        if (key == ID) return new DataRequirement(extensionData.withId(val), type, profile, subject, mustSupport, codeFilter, dateFilter, limit, sort);
        return this;
    }
    ...
    public void serializeAsJsonValue(JsonGenerator generator) throws IOException {
        ...
        if (subject != null) {
            if (subject instanceof CodeableConcept c) {
                c.serializeJsonField(generator, FIELD_NAME_SUBJECT_CODEABLE_CONCEPT);
            } else if (subject instanceof Reference r) {
                r.serializeJsonField(generator, FIELD_NAME_SUBJECT_REFERENCE);
            }
        }
    ...
    public static final class CodeFilter extends AbstractElement implements Complex {
         ...
         * 8 byte - object header
         * 4 byte - extension data reference
         * 4 byte - path reference
         * 4 byte - searchParam reference
         * 4 byte - valueSet reference
    ...
    public static final class DateFilter extends AbstractElement implements Complex {
         ...
         * 4 byte - padding
         */
        private static final int MEM_SIZE_OBJECT = MEM_SIZE_OBJECT_HEADER + 20;

        private static final Keyword FHIR_TYPE = RT.keyword("fhir.DataRequirement", "dateFilter");

        private static final Keyword PATH = RT.keyword(null, "path");
        private static final Keyword SEARCH_PARAM = RT.keyword(null, "searchParam");
        private static final Keyword VALUE_DATE_TIME = RT.keyword(null, "valueDateTime");
        private static final Keyword VALUE_PERIOD = RT.keyword(null, "valuePeriod");
        private static final Keyword VALUE_DURATION = RT.keyword(null, "valueDuration");

        private static final Keyword[] FIELDS = {ID, EXTENSION, PATH, SEARCH_PARAM, VALUE_DATE_TIME, VALUE_PERIOD, VALUE_DURATION};
        ...
        public static DateFilter create(IPersistentMap m) {
            Object value = m.valAt(VALUE_DATE_TIME);
            if (value == null) {
                value = m.valAt(VALUE_PERIOD);
            }
            if (value == null) {
                value = m.valAt(VALUE_DURATION);
            }
        ...
        public Object valAt(Object key, Object notFound) {
            ...
            if (key == PATH) return path;
            if (key == SEARCH_PARAM) return searchParam;
            if (key == VALUE_DATE_TIME) return value instanceof DateTime ? value : null;
            if (key == VALUE_PERIOD) return value instanceof Period ? value : null;
            if (key == VALUE_DURATION) return value instanceof Duration ? value : null;
            return super.valAt(key, notFound);
        }
        ...
        public ISeq seq() {
            ...
            if (value instanceof DateTime) {
                seq = appendElement(seq, VALUE_DATE_TIME, value);
            } else if (value instanceof Period) {
                seq = appendElement(seq, VALUE_PERIOD, value);
            } else if (value instanceof Duration) {
                seq = appendElement(seq, VALUE_DURATION, value);
            }
            seq = appendElement(seq, SEARCH_PARAM, searchParam);
            seq = appendElement(seq, PATH, path);
            return extensionData.append(seq);
        ...
        public DateFilter assoc(Object key, Object val) {
            ...
            if (key == VALUE_DATE_TIME || key == VALUE_PERIOD || key == VALUE_DURATION)
