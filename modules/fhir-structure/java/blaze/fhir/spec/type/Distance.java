...
public final class Distance extends AbstractQuantity {

    private static final Keyword FHIR_TYPE = RT.keyword("fhir", "Distance");

    private static final ILookupThunk FHIR_TYPE_LOOKUP_THUNK = new ILookupThunk() {
        @Override
        public Object get(Object target) {
            return target instanceof Distance ? FHIR_TYPE : this;
        }
    };
    private static final FieldName FIELD_NAME_EXTENSION_VALUE = FieldName.of("valueDistance");

    private static final Distance EMPTY = new Distance(ExtensionData.EMPTY, null, null, null, null, null);

    private static final Interner<InternerKey, Distance> INTERNER = Interners.weakInterner(
            k -> new Distance(k.extensionData(), k.value(), k.comparator(), k.unit(), k.system(), k.code())
    );

    private Distance(ExtensionData extensionData, Decimal value, Code comparator, String unit, Uri system, Code code) {
        super(extensionData, value, comparator, unit, system, code);
    }

    private static Distance maybeIntern(ExtensionData extensionData, Decimal value, Code comparator, String unit,
        ...
        return extensionData.isInterned() && Base.isInterned(value) && Base.isInterned(comparator) &&
                ...
                : new Distance(extensionData, value, comparator, unit, system, code);
    }

    public static Distance create(IPersistentMap m) {
    ...
    @Override
    public Distance assoc(Object key, Object val) {
    ...
    @Override
    public Distance withMeta(IPersistentMap meta) {
