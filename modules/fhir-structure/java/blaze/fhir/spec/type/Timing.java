...
public final class Timing extends AbstractBackboneElement implements Complex, ExtensionValue {
     ...
     * 4 byte - modifierExtension reference
     * 4 byte - event reference
     * 4 byte - repeat reference
    ...
    private static final SerializedString FIELD_NAME_REPEAT = new SerializedString("repeat");
    ...
    public void serializeAsJsonValue(JsonGenerator generator) throws IOException {
        ...
        if (!event.isEmpty()) {
            Primitives.serializeJsonPrimitiveList(event, generator, FIELD_NAME_EVENT);
        }
        if (repeat != null) {
            generator.writeFieldName(FIELD_NAME_REPEAT);
            repeat.serializeAsJsonValue(generator);
        }
        if (code != null) {
            generator.writeFieldName(FIELD_NAME_CODE);
            code.serializeAsJsonValue(generator);
        }
    ...
    public static class Repeat extends AbstractElement implements Complex {
         ...
         * 8 byte - object header
         * 4 byte - extension data reference
         * 4 byte - bounds reference
         * 4 byte - count reference
         * 4 byte - countMax reference
         * 4 byte - duration reference
         * 4 byte - durationMax reference
         * 4 byte - durationUnit reference
         * 4 byte - frequency reference
         * 4 byte - frequencyMax reference
         * 4 byte - period reference
         * 4 byte - periodMax reference
         * 4 byte - periodUnit reference
         * 4 byte - dayOfWeek reference
         * 4 byte - timeOfDay reference
         * 4 byte - when reference
         * 4 byte - offset reference
         */
        private static final int MEM_SIZE_OBJECT = MEM_SIZE_OBJECT_HEADER + 64;
        ...
        private static final SerializedString FIELD_NAME_BOUNDS_DURATION = new SerializedString("boundsDuration");
        private static final SerializedString FIELD_NAME_BOUNDS_RANGE = new SerializedString("boundsRange");
        private static final SerializedString FIELD_NAME_BOUNDS_PERIOD = new SerializedString("boundsPeriod");
        ...
        public void serializeAsJsonValue(JsonGenerator generator) throws IOException {
            ...
            if (bounds != null) {
                switch (bounds) {
                    case Duration boundsDuration -> {
                        generator.writeFieldName(FIELD_NAME_BOUNDS_DURATION);
                        boundsDuration.serializeAsJsonValue(generator);
                    }
                    case Range boundsRange -> {
                        generator.writeFieldName(FIELD_NAME_BOUNDS_RANGE);
                        boundsRange.serializeAsJsonValue(generator);
                    }
                    case Period boundsPeriod -> {
                        generator.writeFieldName(FIELD_NAME_BOUNDS_PERIOD);
                        boundsPeriod.serializeAsJsonValue(generator);
                    }
                    default -> {
                    }
                }
            ...
            if (!dayOfWeek.isEmpty()) {
                Primitives.serializeJsonPrimitiveList(dayOfWeek, generator, FIELD_NAME_DAY_OF_WEEK);
            }
            if (!timeOfDay.isEmpty()) {
                Primitives.serializeJsonPrimitiveList(timeOfDay, generator, FIELD_NAME_TIME_OF_DAY);
            }
            if (!when.isEmpty()) {
                Primitives.serializeJsonPrimitiveList(when, generator, FIELD_NAME_WHEN);
            }
            if (offset != null) {
