...
public final class Url extends PrimitiveElement {
    ...
    public Url(java.lang.String id, List<Extension> extension, java.lang.String value) {
        super(id, extension);
        this.value = value;
    }

    public static Url create(IPersistentMap m) {
        return new Url((java.lang.String) m.valAt(ID), Base.listFrom(m, EXTENSION),
    ...
    public Url empty() {
        return new Url(null, PersistentVector.EMPTY, null);
    }

    @Override
    @SuppressWarnings("unchecked")
    public Url assoc(Object key, Object val) {
        if (key == VALUE) return new Url(id, extension, (java.lang.String) val);
        if (key == EXTENSION) return new Url(id, (List<Extension>) val, value);
        if (key == ID) return new Url((java.lang.String) val, extension, value);
        throw new UnsupportedOperationException("The key `" + key + "` isn't supported on FHIR.Url.");
    }
