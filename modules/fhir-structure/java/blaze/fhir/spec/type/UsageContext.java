...
public final class UsageContext extends AbstractElement implements Complex, ExtensionValue {
    ...
    private static final FieldName FIELD_NAME_VALUE_CODEABLE_CONCEPT = FieldName.of("valueCodeableConcept");
    private static final FieldName FIELD_NAME_VALUE_QUANTITY = FieldName.of("valueQuantity");
    private static final FieldName FIELD_NAME_VALUE_RANGE = FieldName.of("valueRange");
    private static final FieldName FIELD_NAME_VALUE_REFERENCE = FieldName.of("valueReference");
    ...
    private UsageContext(ExtensionData extensionData, Coding code, Element value) {
        super(extensionData);
        this.code = code;
        this.value = value;
    }
    ...
    public static UsageContext create(IPersistentMap m) {
        return new UsageContext(ExtensionData.fromMap(m), (Coding) m.valAt(CODE), (Element) m.valAt(VALUE));
    }
    ...
    public UsageContext assoc(Object key, Object val) {
        ...
        if (key == VALUE)
            return new UsageContext(extensionData, code, (Element) val);
        if (key == EXTENSION)
        ...
        if (value != null) {
            switch (value) {
                case CodeableConcept valueCodeableConcept ->
                        valueCodeableConcept.serializeJsonField(generator, FIELD_NAME_VALUE_CODEABLE_CONCEPT);
                case Quantity valueQuantity ->
                        valueQuantity.serializeJsonField(generator, FIELD_NAME_VALUE_QUANTITY);
                case Range valueRange ->
                        valueRange.serializeJsonField(generator, FIELD_NAME_VALUE_RANGE);
                case Reference valueReference ->
                        valueReference.serializeJsonField(generator, FIELD_NAME_VALUE_REFERENCE);
                default -> {
                }
            }
        }
