(ns blaze.fhir.spec.resource
  ...
  A type-handler in this namespace is a function taking a map of all type
  handlers, a parser and a locator and returns either a FHIR value or an anomaly
  in case of errors.
...
(defn- create-type-handler
  ...
  (when-ok [property-handlers (create-property-handlers type element-definitions)]
    (let [empty-value (create-empty-value type)]
      (fn type-handler [type-handlers parser locator]
...
(def resource-handler
  (fn [type-handlers ^JsonParser parser locator]
      ...
      (if-let [type (findResourceType tree)]
        (if-let [type-handler (get type-handlers (keyword type))]
          (let [parser (TreeTraversingParser. tree (.getCodec parser))]
            ...
            (type-handler type-handlers parser (conj locator type)))
