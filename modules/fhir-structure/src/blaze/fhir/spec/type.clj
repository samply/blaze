(ns blaze.fhir.spec.type
  ...
  (:import
   [blaze.fhir.spec.type Code Coding CodeableConcept Period Identifier DateTime Extension Reference Uri]
...
#_(defextended ExtendedString [id extension value]
  :fhir-type :fhir/string :hash-num 3)

#_(def ^{:arglists '([x])} string
...
#_(def ^{:arglists '([x])} intern-string
...
(def intern-string
  (let [intern (intern/intern-value #(blaze.fhir.spec.type.String. nil nil %))
        intern-extended (intern/intern-value map->String)]
    (fn [data]
      (if (clojure.core/string? data)
        (intern data)
        (let [{:keys [id extension value]} data]
...
(extend-protocol p/FhirType
  blaze.fhir.spec.type.String
  (-type [s]
    (.fhirType s))
  (-interned [s]
    (and (nil? (.id s)) (p/-interned (.extension s)) (nil? (.value s))))
  (-value [s]
    (.value s))
  (-assoc-id [s id]
    (blaze.fhir.spec.type.String. id (.extension s) (.value s)))
  (-assoc-extension [s extension]
    (blaze.fhir.spec.type.String. (.id s) extension (.value s)))
  (-assoc-value [s value]
    (blaze.fhir.spec.type.String. (.id s) (.extension s) value)))
...
(extend-protocol p/FhirType
  ...
  (-type [e]
    (.fhirType e))
  (-interned [e]
    (and (nil? (.id e)) (p/-interned (.extension e)) (p/-interned use)
         (p/-interned type) (nil? value) (p/-interned period)
         (p/-interned assigner)))
  (-has-primary-content [_] true)
  (-serialize-json [e generator]
...
(def reference
  (let [intern (intern/intern-value map->Reference)]
    (fn [{:keys [id extension reference type identifier display] :as data}]
      (if (and (nil? id) (p/-interned extension) (nil? reference) (nil? type)
               (nil? identifier) (nil? display))
        (intern data)
        (Reference. id extension reference type identifier display)))))
