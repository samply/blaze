...
(defn data-requirement
  [& {:keys [id extension type profile subject mustSupport codeFilter dateFilter
      ...
      :or {id (often-nil id-value)
           ...
           mustSupport (gen/vector (string))
           codeFilter (gen/vector (data-requirement-code-filter))
           dateFilter (gen/vector (data-requirement-date-filter))
           limit (nilable (positiveInt))
           sort (gen/vector (data-requirement-sort))}}]
...
(defn dosage
  [& {:keys [id extension modifierExtension sequence text additionalInstruction
      ...
      :or {id (often-nil id-value)
           ...
           text (nilable (string))
           additionalInstruction (gen/vector (codeable-concept))
           ...
           doseAndRate (gen/vector (dosage-dose-and-rate))
...
(defn timing-repeat
  [& {:keys [id extension bounds count countMax duration durationMax durationUnit
      ...
      :or {id (often-nil id-value)
           ...
           dayOfWeek (gen/vector (code))
           timeOfDay (gen/vector (time))
           when (gen/vector (code))
...
(defn trigger-definition
  [& {:keys [id extension type name timing data condition]
      :or {id (often-nil id-value)
           ...
           data (gen/vector (data-requirement))
...
(def-resource-gen activity-definition
  [id id-value
   ...
   relatedArtifact (gen/vector (related-artifact))
   timing (nilable (gen/one-of [(timing) (dateTime) (age) (period) (range) (duration)]))
   dosage (gen/vector (dosage))])
...
(defn- imaging-study-series
  [& {:keys [id extension modifierExtension uid number modality description
      ...
      :or {id (often-nil id-value)
           ...
           endpoint (gen/vector (reference))
           bodySite (nilable (coding))
           laterality (nilable (coding))
           specimen (gen/vector (reference))
           started (nilable (dateTime))
           performer (gen/vector (imaging-study-series-performer))
           instance (gen/vector (imaging-study-series-instance))}}]
...
(def-resource-gen imaging-study
  [id id-value
   ...
   identifier (gen/vector (identifier))
   status (rare-nil (code))
   modality (gen/vector (coding))
   subject (rare-nil (reference :reference (gen/return nil)))
   encounter (nilable (reference :reference (gen/return nil)))
   started (nilable (dateTime))
   basedOn (gen/vector (reference :reference (gen/return nil)))
   referrer (nilable (reference :reference (gen/return nil)))
   interpreter (gen/vector (reference :reference (gen/return nil)))
   endpoint (gen/vector (reference :reference (gen/return nil)))
   numberOfSeries (nilable (unsignedInt))
   numberOfInstances (nilable (unsignedInt))
   procedureReference (nilable (reference :reference (gen/return nil)))
   procedureCode (gen/vector (codeable-concept))
   location (nilable (reference :reference (gen/return nil)))
   reasonCode (gen/vector (codeable-concept))
   reasonReference (gen/vector (reference :reference (gen/return nil)))
   note (gen/vector (annotation))
   description (nilable (string))
   series (gen/vector (imaging-study-series))])
...
(def-resource-gen library
  [id id-value
   ...
   contact (gen/vector (contact-detail))
   description (nilable (markdown))
   useContext (gen/vector (usage-context))
   jurisdiction (gen/vector (codeable-concept))
   ...
   topic (gen/vector (codeable-concept))
   author (gen/vector (contact-detail))
   editor (gen/vector (contact-detail))
   reviewer (gen/vector (contact-detail))
   endorser (gen/vector (contact-detail))
   relatedArtifact (gen/vector (related-artifact))
   parameter (gen/vector (parameter-definition))
   dataRequirement (gen/vector (data-requirement))
...
(defn- observation-reference-range
  [& {:keys [id extension modifierExtension low high type appliesTo age text]
      :or {id (often-nil id-value)
           ...
           high (nilable (quantity))
           type (nilable (codeable-concept))
           appliesTo (gen/vector (codeable-concept))
           age (nilable (range))
           text (nilable (string))}}]
...
(defn- observation-component
  [& {:keys [id extension modifierExtension code value dataAbsentReason interpretation referenceRange]
      :or {id (often-nil id-value)
           ...
           interpretation (gen/vector (codeable-concept))
           referenceRange (gen/vector (observation-reference-range))}}]
...
(def-resource-gen observation
  [id id-value
   ...
   identifier (gen/vector (identifier))
   basedOn (gen/vector (reference :reference (gen/return nil)))
   partOf (gen/vector (reference :reference (gen/return nil)))
   status (rare-nil (code))
   category (gen/vector (codeable-concept))
   code (rare-nil (codeable-concept))
   subject (rare-nil (reference :reference (gen/return nil)))
   focus (gen/vector (reference :reference (gen/return nil)))
   encounter (nilable (reference :reference (gen/return nil)))
   ...
   referenceRange (gen/vector (observation-reference-range))
   hasMember (gen/vector (reference :reference (gen/return nil)))
   derivedFrom (gen/vector (reference :reference (gen/return nil)))
   component (gen/vector (observation-component))])
