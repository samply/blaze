...
describe('bestReadRun', () => {
  it('returns the read run with the maximum IOPS', () => {
    const job = diskPerfJob();
    job.output = [
      readOutput('read-iops', 1, 14100.5, '/s'),
      readOutput('read-iops', 8, 51860.9, '/s'),
    ...
    expect(bestReadRun(toJob(job) as DiskPerfJob)).toMatchObject({
      concurrency: 8,
      iops: 51860.9
    });
  });
  ...
  it('returns undefined without read runs', () => {
    expect(bestReadRun(toJob(diskPerfJob()) as DiskPerfJob)).toBeUndefined();
  });
});
