...
/** Returns the read run with the maximum IOPS over all read runs of `job`, if any. */
export function bestReadRun(job: DiskPerfJob): RandReadRun | undefined {
  return job.readRuns?.reduce<RandReadRun | undefined>(
    (best, run) =>
      run.iops === undefined || (best !== undefined && run.iops <= (best.iops ?? 0)) ? best : run,
    undefined
  );
}
