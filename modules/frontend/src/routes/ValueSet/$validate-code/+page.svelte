...
<main class="mx-auto max-w-7xl py-4 sm:px-6 lg:px-8 flex flex-col gap-4">
	<Form class="mt-4">
		<Section name="Parameters">
			<TextField id="url" name="ValueSet URL" value={form?.url} />
			<TextField id="valueSetVersion" name="ValueSet Version" value={form?.valueSetVersion} />
			<TextField id="code" name="Code" value={form?.code} />
			<TextField id="system" name="System" value={form?.system} />
			<TextField id="systemVersion" name="System Version" value={form?.systemVersion} />
			<TextField id="display" name="Display" value={form?.display} />
			<TextField id="displayLanguage" name="Display Language" value={form?.displayLanguage} />
			<CheckBoxes name="InferSystem">
				<CheckBox id="inferSystem" name="InferSystem" checked={form?.inferSystem ?? false} />
			</CheckBoxes>
		</Section>
		{#snippet buttons()}
