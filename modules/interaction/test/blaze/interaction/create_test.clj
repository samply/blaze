...
(deftest init-test
  ...
  (testing "missing clock"
    (given-failed-system (update config :blaze.interaction/create dissoc :clock)
      :key := :blaze.interaction/create
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :pred] := `(fn ~'[%] (contains? ~'% :clock))))

  (testing "missing rng-fn"
    (given-failed-system (update config :blaze.interaction/create dissoc :rng-fn)
      :key := :blaze.interaction/create
      :reason := ::ig/build-failed-spec
      ...
      (let [{:keys [status headers body]}
            @(handler
              {::reitit/match bundle-match
               :body #fhir/map{:fhir/type :fhir/Bundle
