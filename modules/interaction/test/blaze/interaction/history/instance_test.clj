  ...
  (reitit/router
   [["/Patient"
    ...
    ["/CodeSystem/{id}/_history"
     {:fhir.resource/type "CodeSystem"
      :name :CodeSystem/history-instance}]]
   {:syntax :bracket
    :path context-path}))
  ...
  (reitit/map->Match
   {:data
    {:blaze/base-url ""
     :name :CodeSystem/history-instance
     :fhir.resource/type "CodeSystem"}
    :path (str context-path "/CodeSystem/0/_history")}))
...
(deftest init-test
  (testing "nil config"
    (given-failed-system {:blaze.interaction.history/instance nil}
  ...
  (testing "missing clock"
    (given-failed-system (update config :blaze.interaction.history/instance dissoc :clock)
      :key := :blaze.interaction.history/instance
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :pred] := `(fn ~'[%] (contains? ~'% :clock))))

  (testing "missing rng-fn"
    (given-failed-system (update config :blaze.interaction.history/instance dissoc :rng-fn)
      :key := :blaze.interaction.history/instance
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :pred] := `(fn ~'[%] (contains? ~'% :rng-fn))))

  (testing "missing page-id-cipher"
    (given-failed-system (update config :blaze.interaction.history/instance dissoc :page-id-cipher)
      :key := :blaze.interaction.history/instance
      :reason := ::ig/build-failed-spec
  ...
  (testing "invalid rng-fn"
    (given-failed-system (assoc-in config [:blaze.interaction.history/instance :rng-fn] ::invalid)
      :key := :blaze.interaction.history/instance
      :reason := ::ig/build-failed-spec
  ...
  (testing "invalid page-id-cipher"
    (given-failed-system (assoc-in config [:blaze.interaction.history/instance :page-id-cipher] ::invalid)
      :key := :blaze.interaction.history/instance
      :reason := ::ig/build-failed-spec
  ...
  (testing "returns history with one code system"
    (with-handler [handler]
      ...
      (let [{:keys [status] {[first-entry] :entry :as body} :body}
        ...
        (testing "has a self link"
          (is (= (str base-url context-path "/CodeSystem/0/_history")
                 (link-url body "self"))))
