...
(deftest init-test
  ...
  (testing "missing clock"
    (given-failed-system (update config :blaze.interaction.history/type dissoc :clock)
      :key := :blaze.interaction.history/type
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :pred] := `(fn ~'[%] (contains? ~'% :clock))))

  (testing "missing rng-fn"
    (given-failed-system (update config :blaze.interaction.history/type dissoc :rng-fn)
      :key := :blaze.interaction.history/type
      :reason := ::ig/build-failed-spec
      [:cause-data ::s/problems 0 :pred] := `(fn ~'[%] (contains? ~'% :rng-fn))))

  (testing "missing page-id-cipher"
    (given-failed-system (update config :blaze.interaction.history/type dissoc :page-id-cipher)
      :key := :blaze.interaction.history/type
      :reason := ::ig/build-failed-spec
