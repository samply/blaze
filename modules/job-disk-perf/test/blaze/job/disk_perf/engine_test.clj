...
(deftest check-free-space-test
  (testing "enough space"
    (is (nil? (engine/check-free-space (new-tmp-dir!) (* 4 u/mib)))))

  (testing "not enough space"
    (given (engine/check-free-space (new-tmp-dir!) (bit-shift-left 1 60))
...
(deftest write-phase-test
  (testing "success"
    (let [file (path/resolve (path/path (new-tmp-dir!)) "test.bin")]
  ...
  (testing "writing to a directory fails"
    (given (engine/write-phase! (path/path (new-tmp-dir!)) u/mib noop-progress! not-cancelled)
      ::anom/category := ::anom/fault))
...
(deftest probe-direct-io-test
  ...
  (testing "on a missing file"
    (is (false? (engine/probe-direct-io (path/resolve (path/path (new-tmp-dir!)) "missing.bin"))))))
...
(deftest read-phase-test
  (let [file (path/resolve (path/path (new-tmp-dir!)) "test.bin")]
    ...
    (testing "a missing file fails"
      (given (engine/read-phase! (path/resolve (path/path (new-tmp-dir!)) "missing.bin")
...
(deftest read-sweep-test
  (let [file (path/resolve (path/path (new-tmp-dir!)) "test.bin")]
    ...
    (testing "a missing file fails"
      (given (engine/read-sweep! (path/resolve (path/path (new-tmp-dir!)) "missing.bin")
...
(deftest fsync-phase-test
  (let [file (path/resolve (path/path (new-tmp-dir!)) "wal.bin")
    ...
    (testing "writing to a directory fails"
      (given (engine/fsync-phase! (path/path (new-tmp-dir!)) {:duration-millis 200}
                                  noop-progress! not-cancelled)
        ::anom/category := ::anom/fault))))
