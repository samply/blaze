...
(deftest init-test
  ...
  (doseq [key [:admin-node :executor :block-size :clock]]
    (testing (str "missing " (name key))
      (given-failed-system (update (config (new-tmp-dir!)) :blaze.job/disk-perf dissoc key)
        :key := :blaze.job/disk-perf
        :reason := ::ig/build-failed-spec
        [:cause-data ::s/problems 0 :pred] := `(fn ~'[%] (contains? ~'% ~key)))))

  (testing "invalid dirs"
    (given-failed-system (assoc-in (config (new-tmp-dir!)) [:blaze.job/disk-perf :dirs] ::invalid)
      :key := :blaze.job/disk-perf
      :reason := ::ig/build-failed-spec
