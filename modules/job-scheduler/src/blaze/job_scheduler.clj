  ...
  (-> (job-util/pull-job node id)
      (ac/then-compose
       (fn [{:keys [status] :as job}]
         (condp = (:value status)
           "in-progress" (job-util/update-job node job hold-job** reason)
           "on-hold" (ac/completed-future job)
...
(defn- resume-job* [{:keys [node]} id]
  (-> (job-util/pull-job node id)
      (ac/then-compose
       (fn [{:keys [status] :as job}]
         (condp = (:value status)
           "on-hold" (job-util/update-job node job resume-job**)
