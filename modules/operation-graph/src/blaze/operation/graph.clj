...
(defn- resource-identity [{:fhir/keys [type] :keys [id]}]
  (str (name type) "/" id))
...
(defn- link-params-resource-handles [clauses]
  (fn [db source-resource target-node]
    (let [ref (resource-identity source-resource)]
...
(defn- process-node
  ...
  [db graph node processed-resources resource-handle]
  (-> processed-resources
      (ac/then-compose
       (fn [processed-resources]
         (let [resource-identity (resource-identity resource-handle)]
