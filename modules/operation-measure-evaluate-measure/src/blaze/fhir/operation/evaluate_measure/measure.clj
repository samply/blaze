...
(defn- canonical-clauses
  "Splits `library-ref` into a `url` search clause and, when present, a
  `version` search clause, following the `url|version` canonical reference
  syntax (FHIR R4 §2.3.0.5)."
  [library-ref]
  (let [[url version] (fu/split-canonical library-ref)]
...
(defn- find-library-by-canonical
  ...
  if none was found."
  [db library-ref]
  (do-sync [libraries (-> (d/type-query db "Library" (canonical-clauses library-ref))
                          (ac/then-compose (fn [handles] (d/pull-many db (vec handles)))))]
    (first (fu/sort-by-priority libraries))))
