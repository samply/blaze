...
(def ^:private system-clock-config
  (-> (assoc-in config [::tx-log/local :clock]
...
(deftest since-test
  (with-system-data [{node :blaze.db/node
    ...
    (let [start Instant/EPOCH
          ...
          _ @(d/transact node [[:put {:fhir/type :fhir/Observation :id "2"
                                      :subject #fhir/Reference{:reference "Patient/0"}}]])
          {:keys [status]
           {[first-entry second-entry third-entry] :entry :as body} :body}
          @(handler {:path-params {:id "0"}
                     :query-params {"_since" (.format DateTimeFormatter/ISO_INSTANT start)}})]

      (testing "start of time"
        (is (= 200 status))
      ...
      (testing "after initialization"
        (let [{:keys [status] {[first-entry second-entry] :entry :as body} :body}
              @(handler {:path-params {:id "0"}
                         :query-params {"_since" (.format DateTimeFormatter/ISO_INSTANT after)}})]
                    ...
                    @(handler
                      {::reitit/match page-match
                       ...
                       (page-path-params
                        page-id-cipher
                        {"_count" "1" "__t" "2"
                         "_since" (.format DateTimeFormatter/ISO_INSTANT after)
                         "__page-offset" "1"})})]
