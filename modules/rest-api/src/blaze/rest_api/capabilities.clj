...
(defn capabilities-handler
  "Returns a Ring handler that outputs the capability statement."
  {:arglists '([context])}
  [{:keys [context-path] :or {context-path ""} :as context}]
  (let [capability-statement-base (build-capability-statement-base context)]
