...
(defn- process-batch-entry-retrying
  ...
  So it has to stay narrowed to the `::anom/busy` anomaly `ac/retry` retries.
  An `::anom/unavailable` of a closing node or a timeout waiting for a t can
  mean that the transaction was applied after all."
  [context idx entry]
