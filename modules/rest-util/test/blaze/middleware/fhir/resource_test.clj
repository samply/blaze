...
(defn- binary-resource-as-json [b64-encoded-binary-data content-type]
  (str "{\"data\" : \"" b64-encoded-binary-data "\", \"resourceType\" : \"Binary\", \"contentType\" : \"" content-type "\"}"))

(defn- binary-resource-as-xml [b64-encoded-binary-data content-type]
  ...
  (testing "when sending FHIR-wrapped (base64-encoded) binary data"
    (testing "both handlers should return the same for both wrappers (JSON and XML)"
      (let [b64-encoded-binary-data "MTA1NjE0Cg=="
        ...
        (doseq [handler [resource-body-handler binary-resource-body-handler]
                [fhir-content-type resource-string-representation]
                [["application/fhir+json;charset=utf-8" (binary-resource-as-json b64-encoded-binary-data binary-resource-content-type)]
                 ["application/fhir+xml;charset=utf-8" (binary-resource-as-xml b64-encoded-binary-data binary-resource-content-type)]]]
          (let [closed? (atom false)]
            (given @(handler
