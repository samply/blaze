...
(deftest estimate-storage-size-test
  (with-system [{db ::kv/rocksdb} (config (new-temp-dir!))]
    (is (zero? (kv/estimate-storage-size db :default full-key-range)))

    (given (kv/estimate-storage-size db :foo full-key-range)
      ::anom/category := ::anom/not-found
      ::anom/message := "Column family `foo` not found."))
  ...
  (with-system [{db ::kv/rocksdb} (config (new-temp-dir!))]
    ...
    (is (pos? (kv/estimate-storage-size db :default full-key-range)))))
