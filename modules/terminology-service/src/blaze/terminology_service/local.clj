...
(defmethod m/pre-init-spec ::ts/local [_]
  (s/keys :req-un [:blaze.db/node :blaze/clock ::sct-release-path]))

(defmethod ig/init-key ::ts/local
  [_ {:keys [node clock sct-release-path]}]
  (log/info "Init local terminology server")
  (if-ok [sct-context (sct/build-context sct-release-path)]
    (let [context {:clock clock :sct/context sct-context}]
        ...
        (-code-system-validate-code [_ request]
          (let [db (d/new-batch-db (d/db node))]
            (-> (find-code-system (assoc context :db db) request)
                (ac/then-apply
                 #(cs/validate-code % (assoc context :request request)))
                (handle-close db))))
