...
(defn- cs-validate-code-more
  ...
  [{:keys [url code-system code coding codeable-concept] :as params}]
  (if-not (and url code-system)
    (cond
      code
      (if (or url code-system)
        (assoc params :clause {:code code})
      ...
      (let [{:keys [coding]} codeable-concept]
        (condp = (count coding)
          1 (when-ok [clause (cs-coding-clause (first coding) "codeableConcept")]
              (assoc params :clause clause))
