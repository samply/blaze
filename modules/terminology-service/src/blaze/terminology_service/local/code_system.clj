...
(defn- find-in-tx-resources
  ([{:keys [tx-resources] ::cs/keys [required-content]
     :or {required-content "complete"}} url]
   (some
    ...
    tx-resources))
  ([{:keys [tx-resources] ::cs/keys [required-content]
     :or {required-content "complete"}} url version]
   (some
    (fn [{:fhir/keys [type] :as resource}]
      (when (identical? :fhir/CodeSystem type)
        (when (= url (type/value (:url resource)))
...
(defn satisfies-filter?
  "Returns true if `concept` satisfies `filter` or an anomaly in case of errors."
  [code-system filter concept]
  (c/satisfies-filter? code-system filter concept))
