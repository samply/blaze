...
(def locales (into {} (map locale-details) (Locale/getAvailableLocales)))
...
(defmethod c/expand-concept :bcp-47
  ...
  (into
   ...
   (keep
    (fn [{:keys [code]}]
      (when-let [display-code (display (type/value code))]
...
(defmethod c/find-complete :bcp-47
  ...
  (when-let [display-code (display code)]
    {:code (type/code code) :display display-code :system url :version version}))
...
(comment
  (count locales)
  (some #{"und"} (keys locales))
  (get locales "de")
  (get locales "de-DE")
  (get locales "dje-Latn-NE")
  (locale-details (Locale. "de"))
  (locale-details (Locale. "de", "DE"))
  (locale-details (Locale. "dje", "NE", "Latn"))
  (display "de")
  (display "de-DE")
  (display "dje-Latn-NE"))
