...
(defmethod c/find-complete :sct
  [{{:keys [concept-index]} :sct/context :sct/keys [module-id version]
    :as code-system} {{:keys [code]} :clause}]
...
(defn build-context
  "Builds the :sct/context from files in `release-path`."
  [release-path]
