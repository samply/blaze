...
(defn- infer-non-unique-msg [{:keys [url]} {:keys [code]}]
  (if url
    (format "While inferring the system was requested, the provided code `%s` was not unique in the value set with URL `%s`." code url)
...
(defn- not-found-msg [{:keys [url]} code system]
  (if url
    (format "The provided code `%s` of system `%s` was not found in the value set with URL `%s`." code system url)
...
(defn validate-code [{:keys [request] :as context} value-set]
