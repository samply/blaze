...
(defn- value-set-canonical [{:keys [url version]}]
  (cond-> (type/value url) (type/value version) (str "|" (type/value version))))

(defn not-in-vs-msg [value-set {:keys [code system]}]
  (format "The provided code `%s` was not found in the value set `%s`."
          (cond->> code system (str system "#")) (value-set-canonical value-set)))

(defn not-in-vs [value-set {:keys [origin] :as clause}]
  ...
  {:fhir/type :fhir.OperationOutcome/issue
   ...
   (type/map->CodeableConcept
    {:coding [invalid-code-coding]
     :text (type/string (format "Unknown code `%s` was not found in the system `%s`." code (cond-> system version (str "|" version))))})
   :expression [(type/string (cond->> "code" origin (str origin ".")))]})
  ...
  {:fhir/type :fhir.OperationOutcome/issue
   ...
   (type/map->CodeableConcept
    {:coding [vs-invalid-coding]
     :text (type/string (format "Unable to check whether the code is in the value set `%s` because the value set `%s` was not found."
                                (value-set-canonical value-set) unknown-value-set-url))})})
...
(defn unknown-code-system
  ...
  {:fhir/type :fhir.OperationOutcome/issue
   ...
   (type/map->CodeableConcept
    {:coding [vs-invalid-coding]
     :text (type/string (format "Unable to check whether the code is in the value set `%s` because the code system `%s` was not found."
                                (value-set-canonical value-set) (cond-> system version (str "|" version))))})})
