...
(deftest init-test
  ...
  (testing "missing config"
    (given-thrown (ig/init {::ts/local {}})
      ...
      [:cause-data ::s/problems 2 :pred] := `(fn ~'[%] (contains? ~'% :sct-release-path))))
  ...
  (testing "invalid node"
    (given-thrown (ig/init {::ts/local {:node ::invalid}})
      ...
      [:cause-data ::s/problems 1 :pred] := `(fn ~'[%] (contains? ~'% :sct-release-path))
...
(def config
  (assoc
   ...
   {:node (ig/ref :blaze.db/node)
    ...
    :sct-release-path "sct-release-specimen"}
...
(deftest code-system-validate-code-ucum-test
  (testing "with id or url"
    (with-system-data [{ts ::ts/local} config]
      ...
      (testing "existing code"
        (let [request {:code "s"}]
          (doseq [request (mapv (partial merge request) [{:url "http://unitsofmeasure.org"} {:id "0"}])]
            (given @(ts/code-system-validate-code ts request)
              :fhir/type := :fhir/Parameters
              [:parameter count] := 3
              [:parameter 0 :name] := #fhir/string"result"
...
(deftest code-system-validate-code-sct-test
  (with-system-data [{ts ::ts/local} config]
    [[[:put {:fhir/type :fhir/CodeSystem :id "0"
             ...
             :version #fhir/string"http://snomed.info/sct/900000000000207008/version/20240701"
             :content #fhir/code"not-present"}]]]

    (testing "existing code"
      (let [request {:code "441510007"}]
        (doseq [request (mapv (partial merge request) [{:url "http://snomed.info/sct"} {:id "0"}])]
          (given @(ts/code-system-validate-code ts request)
            :fhir/type := :fhir/Parameters
            [:parameter count] := 3
            [:parameter 0 :name] := #fhir/string"result"
...
(deftest expand-value-set-value-set-refs-test
...
(deftest expand-value-set-sct-test
    ...
    (with-system-data [{ts ::ts/local} config]
      ...
      (doseq [request [{} {:system-version [#fhir/canonical"http://snomed.info/sct|http://snomed.info/sct/900000000000207008"]}]]
        (given @(ts/expand-value-set ts (assoc request :id "0"))
          ...
          [:expansion :contains 0 :code] := #fhir/code"57921000052103"
          [:expansion :contains 0 :display] := #fhir/string"Whole blood specimen with edetic acid (specimen)"
          ...
          [:expansion :contains 2 :code] := #fhir/code"445295009"
          [:expansion :contains 2 :display] := #fhir/string"Blood specimen with edetic acid (specimen)"))))
    ...
    (with-system-data [{ts ::ts/local} config]
      ...
      (doseq [request [{} {:system-version [#fhir/canonical"http://snomed.info/sct|http://snomed.info/sct/900000000000207008"]}]]
        (given @(ts/expand-value-set ts (assoc request :id "0"))
          ...
          [:expansion :contains 0 :display] := #fhir/string"Whole blood specimen with edetic acid (specimen)"
          [:expansion :contains 1 :system] := #fhir/uri"http://snomed.info/sct"
          [:expansion :contains 1 :code] := #fhir/code"445295009"
          [:expansion :contains 1 :display] := #fhir/string"Blood specimen with edetic acid (specimen)")))))
            ...
            (let [request {:code "code-115927" :infer-system true}]
              (doseq [request (mapv (partial merge request) [{:url "value-set-203901"} {:id "1"}])]
                (given @(ts/value-set-validate-code ts request)
                  ...
                  [:parameter 0 :value] := #fhir/boolean false
                  [:parameter 1 :name] := #fhir/string"message"
                  [:parameter 1 :value] := #fhir/string"While inferring the system was requested, the provided code `code-115927` was not unique in the value set with URL `value-set-203901`."))))))
        ...
        (let [request {:code "code-153948" :system "system-115910"}]
          (doseq [request (mapv (partial merge request) [{:url "value-set-135750"} {:id "0"}])]
            (given @(ts/value-set-validate-code ts request)
              ...
              [:parameter 0 :value] := #fhir/boolean false
              [:parameter 1 :name] := #fhir/string"message"
              [:parameter 1 :value] := #fhir/string"The provided code `code-153948` of system `system-115910` was not found in the value set with URL `value-set-135750`."))))
