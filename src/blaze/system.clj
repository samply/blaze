...
(defn init!
  ...
  (if-ok [{:keys [bind-map settings]}
          (ce/bindings-and-settings
           ...
           'db-scaling-factor ["DB_SCALING_FACTOR" ce/coerce-db-scaling-factor "1"]
...
(defmethod ig/expand-key :blaze.db.kv/rocksdb
  [key {:keys [scaling-factor] :or {scaling-factor 1} :as config}]
  {key (cond-> config
         (< 1 scaling-factor)
         (update :column-families scale-column-families scaling-factor))})
